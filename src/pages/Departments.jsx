...
import { People, TrendingUp, ChevronRight, DeleteOutline, Close } from "@mui/icons-material";
...
export const Departments = () => {
  ...
  const filteredDepts = useMemo(() => {
    if ((isEmployee || isManager) && currentDepartmentName) {
      return departments.filter((department) => department.name === currentDepartmentName);
    }
    return departments;
  }, [currentDepartmentName, departments, isEmployee, isManager]);
  ...
  const handleDeleteDepartment = async (department) => {
    if (!canManageDepartments || !department) {
      return;
    }
    ...
    try {
      const result = await workspaceService.deleteDepartment(department.id);
      if (!result.success) {
        setDepartmentError(result.error || "Не удалось удалить отдел.");
        return;
      }
      ...
      setDepartmentSuccess("Отдел удалён.");
      await refreshWorkspaceData();
    } catch (error) {
      setDepartmentError(error.response?.data?.error || "Не удалось удалить отдел.");
    } finally {
  ...
  return (
    <div className="departments-page">
      ...
      ) : departmentSuccess || departmentError ? (
        <section className="workspace-toolbar">
          {departmentSuccess ? <div className="workspace-success">{departmentSuccess}</div> : null}
          {departmentError ? <div className="workspace-empty">{departmentError}</div> : null}
        </section>
        ...
        <div className="modal-overlay" onClick={() => setShowCreateDepartmentModal(false)}>
          <div className="modal-card" onClick={(event) => event.stopPropagation()}>
            <div className="modal-header">
              <div>
                ...
                <p className="appeal-create-description">
                  Отдел появится в базе данных и сразу станет доступен в системе.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="appeal-form-grid">
              ...
              <label className="appeal-form-field">
                ...
                <select
                  ...
                  onChange={handleDepartmentChange("headId")}
            ...
            <div className="appeal-form-actions">
              ...
              >
                {isCreatingDepartment ? "Сохранение..." : "Создать отдел"}
              </button>
              {departmentError ? <div className="workspace-empty">{departmentError}</div> : null}
            </div>
          </div>
      ...
      <div className="departments-content">
            ...
            >
              <div className="dept-card-header">
                <div className="dept-card-info">
                  <div className="dept-card-icon" style={{ backgroundColor: `${department.color}20` }}>
                    <div className="dept-card-dot" style={{ backgroundColor: department.color }} />
                  </div>
                  <div>
                ...
                <ChevronRight
                  sx={{ fontSize: 16 }}
                  className={`dept-card-arrow ${selectedDept?.id === department.id ? "dept-card-arrow-rotated" : ""}`}
                />
              </div>
              ...
              <div className="dept-card-stats">
                ...
                <div className="dept-card-stat">
                  ...
                  <div
                    className={`dept-card-stat-kpi ${
                      department.kpi >= 85 ? "kpi-high" : department.kpi >= 75 ? "kpi-medium" : "kpi-low"
                    }`}
                  >
                    {department.kpi}%
                  </div>
              ...
              <div className="dept-card-kpi-bar">
                <div
                  className="dept-card-kpi-fill"
                  style={{ width: `${department.kpi}%`, backgroundColor: department.color }}
                />
              </div>
            </div>
          ))}
        ...
        <div className="dept-detail-panel">
          {selectedDept ? (
            <div>
              <div className="dept-detail-header dept-detail-header-actions">
                <div className="dept-detail-header-main">
                  <div className="dept-detail-icon" style={{ backgroundColor: `${selectedDept.color}20` }}>
                    <div className="dept-detail-dot" style={{ backgroundColor: selectedDept.color }} />
                  </div>
                  <div>
                ...
                {canManageDepartments ? (
                  ...
                  >
                    ...
                    {isDeletingDepartment ? "Удаление..." : "Удалить отдел"}
                  </button>
                ) : null}
              </div>
                ...
                <div className="detail-row">
                  ...
                  <span
                    className={`detail-kpi ${
                      selectedDept.kpi >= 85 ? "kpi-high" : selectedDept.kpi >= 75 ? "kpi-medium" : "kpi-low"
                    }`}
                  >
                    {selectedDept.kpi}%
                  </span>
                ...
                <div className="dept-employees-list">
                  {deptEmployees.map((employee) => (
                    <div key={employee.id} className="dept-employee-item">
                      ...
                      <span
                        className={`dept-employee-kpi ${
                          employee.kpi >= 90 ? "kpi-high" : employee.kpi >= 75 ? "kpi-medium" : "kpi-low"
                        }`}
                      >
                        {employee.kpi}%
                      </span>
                ...
                <tr key={department.id} className="summary-table-row">
                  <td className="summary-table-cell">
                    <div className="summary-dept-name">
                      <div className="summary-dept-dot" style={{ backgroundColor: department.color }} />
